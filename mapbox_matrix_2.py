...
class TSPSolver:
    ...
    def solve_tsp(self, 
        ...
        if len(valid_locations) <= 12:
            print("🔍 Using Held-Karp algorithm for exact solution...")
            best_route = self._solve_tsp_held_karp(
                valid_locations, 
                distance_lookup, 
                duration_lookup,
                start_location,
            ...
            print("🔍 Using nearest neighbor heuristic for large problem...")
            best_route = self._solve_tsp_nearest_neighbor(
                valid_locations, 
                distance_lookup, 
                duration_lookup,
                start_location,
    ...
    def _solve_tsp_brute_force(self, 
                               ...
                               return_to_start: bool = True) -> Optional[Route]:
        """
        Solve TSP using brute force (for very small numbers of locations).
        ...
        """
        if len(locations) > 8:
            print("⚠️  Too many locations for brute force. Consider using Held-Karp algorithm...")
            return None
    ...
    def _solve_tsp_held_karp(self, 
        ...
        This is an exact algorithm that finds the optimal solution using dynamic programming.
        ...
        # Create location indices for easier handling
        location_to_idx = {loc: i for i, loc in enumerate(locations)}
        idx_to_location = {i: loc for i, loc in enumerate(locations)}
        
        # Handle start and end location constraints
        start_idx = location_to_idx.get(start_location, 0) if start_location else 0
        end_idx = location_to_idx.get(end_location, 0) if end_location else start_idx
        ...
        # Base case: starting from start_idx
        initial_mask = 1 << start_idx
        dp[(initial_mask, start_idx)] = 0
        
        # Iterate through all possible subsets of cities
        for mask in range(1 << n):
            # Only consider masks that include the start city
            if not (mask & (1 << start_idx)):
                continue
                
            for pos in range(n):
                # Only consider positions that are in the current mask
                if not (mask & (1 << pos)):
                    continue
                
                current_state = (mask, pos)
                if current_state not in dp:
                    continue
                
                current_cost = dp[current_state]
                
                # Try to extend the path to all unvisited cities
                for next_pos in range(n):
                    if mask & (1 << next_pos):  # Already visited
                        continue
                    
                    # Check if we can go from pos to next_pos
                    from_loc = idx_to_location[pos]
                    to_loc = idx_to_location[next_pos]
                    
                    if distance_lookup[from_loc][to_loc] is None:
                        continue
                    
                    new_mask = mask | (1 << next_pos)
                    new_state = (new_mask, next_pos)
                    new_cost = current_cost + distance_lookup[from_loc][to_loc]
                    
                    if new_state not in dp or new_cost < dp[new_state]:
                        dp[new_state] = new_cost
                        parent[new_state] = pos
        
        # Find the best final state
        best_cost = float('inf')
        best_final_state = None
        
        # Consider all possible ending positions
        final_mask = (1 << n) - 1  # All cities visited
        
        for pos in range(n):
            final_state = (final_mask, pos)
            if final_state in dp:
                cost = dp[final_state]
                
                # Add cost to return to start if required
                if return_to_start and pos != start_idx:
                    from_loc = idx_to_location[pos]
                    to_loc = idx_to_location[start_idx]
                    if distance_lookup[from_loc][to_loc] is not None:
                        cost += distance_lookup[from_loc][to_loc]
                
                # Add cost to reach end location if specified and different
                if end_location and pos != end_idx:
                    from_loc = idx_to_location[pos]
                    to_loc = idx_to_location[end_idx]
                    if distance_lookup[from_loc][to_loc] is not None:
                        cost += distance_lookup[from_loc][to_loc]
        ...
        while current_state in parent:
            mask, pos = current_state
            path.append(idx_to_location[pos])
            prev_pos = parent[current_state]
            prev_mask = mask & ~(1 << pos)
            current_state = (prev_mask, prev_pos)
        
        # Add the starting position
        path.append(idx_to_location[start_idx])
        path.reverse()
        
        # Handle end location constraint
        if end_location and path[-1] != end_location:
            if return_to_start:
                # Insert end location before returning to start
                path.insert(-1, end_location)
            else:
